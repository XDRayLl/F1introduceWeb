...
function ShowTime({ races }) {
  ...
  for (var i = 0; i < upcomingRaceDate.length; i++) {
    if (upcomingRace == upcomingRaceDate[i]) {
      ...
      let AllRaceTime = races.map((e) => e.time);
      upcomingRaceName = AllRaceName[i];
      RaceDate = AllRaceDate[i];
      RaceRound = AllRaceRound[i];
      RaceTime = AllRaceTime[i];
    }
  }
  // console.log("race.date:", RaceTime);
  let TaipeiRaceTime = moment.utc(RaceDate + "T" + RaceTime).tz("Asia/Taipei");
    ...
    <div>
      ...
      <div className="PreviewArea">
        <div className="PreviewLeft">
          <p className="Round">
            R{RaceRound} | {moment(RaceDate).format("MMM Do")}
          </p>
          <div className="RaceName">
            <GPFlag value={RaceRound} />
            {upcomingRaceName}
          </div>
          <p className="Countdown">
            {days}
            <span className="unit">D</span> {hours}
            <span className="unit">H</span> {minutes}
            <span className="unit">M</span> {seconds}
            <span className="unit">s</span>
          </p>
        </div>
        <div className="PreviewRight">
          <div className="TimeArea">
            <p className="Mytime">MY Time</p>
            <p>
              {currentTime.getHours()}:{currentTime.getMinutes()}
            </p>
          </div>
          <div className="TimeArea">
            <p>TRACK TIME&nbsp;&nbsp;{TaipeiRaceTime.format("HH:mm")}</p>
          </div>
        </div>
      </div>
    </div>
...
function GPFlag({ value }) {
  const flag = {
    ...
    11: (
      <>
        <div className="flag">
          <img
            src="data:image/png;base64,"
            alt=""
          />
        </div>
      </>
    ...
    24: (
      <>
        <div className="flag">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/thumb/c/cb/Flag_of_the_United_Arab_Emirates.svg/250px-Flag_of_the_United_Arab_Emirates.svg.png"
            alt=""
          />
        </div>
      </>
  ...
  return flag[value] || <div>未知</div>;
}
export default RaceDataAPI;
